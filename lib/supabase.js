...
export async function checkSupabaseConnection() {
  ...
  try {
    ...
    const [tableResponse, jobsResponse, itemsResponse] = await Promise.all([
      fetch(
        base +
      ...
      fetch(
        base +
          "/rest/v1/qualification_jobs?select=id,status,industry,total_items&limit=0",
        {
          headers: supabaseHeaders(config, {
            Accept: "application/json"
          })
      ...
      fetch(
        base +
          "/rest/v1/qualification_job_items?select=id,job_id,status,stage&limit=0",
        {
          headers: supabaseHeaders(config, {
            Accept: "application/json"
          })
    ...
    const [tableBody, jobsBody, itemsBody] = await Promise.all([
      tableResponse.text(),
      jobsResponse.text(),
      itemsResponse.text()
    ]);
    ...
    if (!jobsResponse.ok || !itemsResponse.ok) {
      const raw = !jobsResponse.ok ? jobsBody : itemsBody;
      ...
      try {
        ...
        queueDiagnostic = {
          ...
          message:
            parsed.message ||
            "Qualification queue schema check failed."
        };
      } catch {
        queueDiagnostic = {
          code: null,
          message: "Qualification queue schema check failed."
        };
      }
    }

    const queueReady = jobsResponse.ok && itemsResponse.ok;
    ...
    return {
      ...
      qualificationQueueReady: queueReady,
      qualificationJobsStatus: jobsResponse.status,
      qualificationItemsStatus: itemsResponse.status,
      keyType,
      diagnostic: tableDiagnostic || queueDiagnostic
    };
  } catch (error) {
    return {
      configured: true,
...
export async function saveContactResolution(prospect, resolution) {
  ...
  const now = new Date().toISOString();
  const website = prospect.website || resolution.website;
