...
function safeError(error) {
  const stage = error?.agentStage ? "[" + error.agentStage + "] " : "";
  const message = String(error?.message || "Unknown qualification error.");
  return (stage + message).slice(0, 1200);
}
...
function aggregateCounts(items) {
  ...
  for (const item of items) {
    ...
    if (item.enrichment) counts.enriched += 1;
    if (item.scoring) counts.scored += 1;
    if (item.contact_resolution) counts.contactResolved += 1;
    if (item.outreach_package) counts.outreachDrafted += 1;
  }
...
async function persistBestEffort(label, operation, warnings) {
  try {
    await operation();
  } catch (error) {
    const message = label + " persistence warning: " + safeError(error);
...
function warningText(warnings) {
  return warnings.length ? warnings.join(" | ").slice(0, 1200) : null;
}

function terminalFailureState(currentStage) {
  if (currentStage === "CONTACT_RESOLVING") {
    return { status: "PARTIAL", stage: "CONTACT_FAILED" };
  }

  if (currentStage === "OUTREACH_DRAFTING") {
    return { status: "PARTIAL", stage: "OUTREACH_FAILED" };
  }

  if (currentStage === "SCORING") {
    return { status: "PARTIAL", stage: "SCORING_FAILED" };
  }

  return { status: "FAILED", stage: "ENRICHMENT_FAILED" };
}
...
async function processItem(job, item) {
  ...
  try {
    ...
    if (
      !enrichment ||
      ["ENRICHMENT_QUEUED", "ENRICHING"].includes(currentStage)
    ) {
      ...
      await persistBestEffort(
        "Agent 2 prospect",
        () => upsertProspectEnrichment(prospect, enrichment),
        warnings
      );
      ...
      await updateQualificationJobItem(item.id, {
        stage: currentStage,
        enrichment,
        last_error: warningText(warnings)
      });
    }

    if (
      !scoring ||
      ["ENRICHED", "SCORING"].includes(currentStage)
    ) {
      ...
      await persistBestEffort(
        "Agent 3 prospect",
        () => saveProspectScore(prospect, scoring),
        warnings
      );
    ...
    if (
      !contactResolution ||
      ["SCORED", "CONTACT_RESOLVING"].includes(currentStage)
    ) {
      ...
      await persistBestEffort(
        "Agent 4 prospect",
        () => saveContactResolution(prospect, contactResolution),
        warnings
      );

      currentStage = "CONTACT_RESOLVED";
      await updateQualificationJobItem(item.id, {
        stage: currentStage,
    ...
    if (shouldDraft) {
      ...
      await persistBestEffort(
        "Agent 5 prospect",
        () => saveOutreachPackage(prospect, outreach),
        warnings
      );
    ...
    console.error(
      "Qualification item failed at " + currentStage + ":",
      item.company_name,
      error
    );
    ...
    const terminalState = terminalFailureState(currentStage);

    await updateQualificationJobItem(item.id, {
      status: canRetry ? "QUEUED" : terminalState.status,
      stage: canRetry ? retryStage : terminalState.stage,
  ...
  try {
    ...
    while (true) {
      ...
      const pending = (job.items || []).filter(
        (candidate) => !TERMINAL_ITEM_STATUSES.has(candidate.status)
      );
      ...
      for (let i = 0; i < pending.length; i += concurrency) {
        ...
        await Promise.allSettled(
          batch.map((candidate) => processItem(job, candidate))
        );
