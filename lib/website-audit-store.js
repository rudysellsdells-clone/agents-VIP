...
function fromPageRow(row) {
  ...
  return {
    ...
    headingOrderIssues: row.heading_order_issues,
    wordCount: row.word_count,
    publishedDate: row.published_date,
    modifiedDate: row.modified_date,
    estimatedContentAgeDays: row.estimated_content_age_days,
    contentStale: row.content_stale,
    internalLinks: row.internal_links,
    externalLinks: row.external_links,
    images: row.images,
    missingAltImages: row.missing_alt_images,
    forms: row.forms,
    primaryCta: row.primary_cta,
    hasViewport: row.has_viewport,
    schemaTypes: row.schema_types || [],
    cmsSignals: row.cms_signals || [],
    contentQualityScore: row.content_quality_score,
    conversionScore: row.conversion_score,
    technicalSeoScore: row.technical_seo_score,
...
export async function getWebsiteAuditById(auditId, { includePages = true } = {}) {
  const config = getSupabaseConfig();
  const { url, key } = config;
  if (!url || !key) {
  ...
  if (!auditResponse.ok) {
    throw new Error("Website audit lookup failed (" + auditResponse.status + ").");
  }

  const rows = await auditResponse.json();
  const audit = fromAuditRow(rows[0] || null);
  if (!audit || !includePages) return audit;

  const pagesResponse = await fetch(
    base +
      "/rest/v1/website_audit_pages?audit_id=eq." +
      encodeURIComponent(auditId) +
  ...
  if (!pagesResponse.ok) {
    throw new Error(
      "Website audit page lookup failed (" + pagesResponse.status + ")."
    );
  }

  audit.pages = (await pagesResponse.json()).map(fromPageRow).filter(Boolean);
  return audit;
}
...
export async function saveWebsiteAudit(prospect, audit) {
  ...
  const row = {
    ...
    website: normalizedProspectWebsite,
